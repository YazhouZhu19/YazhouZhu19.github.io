...
import { PreliminaryAnalysis as PreliminaryAnalysisView } from "./preliminary-analysis";
import { PaperPreliminaryAnalysis } from "./paper-analysis";
import { buildDashboard, defaultDashboardFilters, type DashboardFilters, type DashboardDrill } from "@/lib/dashboard";
const nav = [{ id: "dashboard", label: "科研仪表盘", icon: LayoutDashboard }, { id: "analysis", label: "初步分析", icon: Workflow }, { id: "feed", label: "论文动态", icon: Activity }, { id: "trends", label: "方向趋势", icon: ChartNoAxesCombined }, { id: "teams", label: "团队观察", icon: Users }, { id: "saved", label: "选题收藏", icon: Bookmark }, { id: "sources", label: "数据与同步", icon: Database }];
...
export default function MonitorWorkspace() {
    ...
    const [analysisFilters, setAnalysisFilters] = useState<DashboardFilters>(defaultDashboardFilters);
    const [drillOrigin, setDrillOrigin] = useState<"dashboard" | "analysis">("dashboard");
    ...
    const chooseTrack = (id: string, leaveDrill = false) => { if (leaveDrill) setDashboardDrill(null); setTrack(id); if (id !== "human_loop") setHuman("all"); };
    const openDrill = (drill: DashboardDrill, origin: "dashboard" | "analysis" = "dashboard") => { reset(); setDrillOrigin(origin); setDashboardDrill({...drill,label:tr(drill.label)}); setView("feed"); setSort("discovered"); };
    ...
    const exportCsv = () => {
        ...
        const rows = [["标题", "作者", "来源日期", "来源", "状态", "DOI", "原文链接", "感兴趣方向", "任务标签", "模态", "笔记"].map(v => tr(v)), ...filtered.map(p => [p.title, p.authors.map(a => a.name).join("; "), p.publishedAt, p.sources.join("; "), p.status, p.doi, p.url, p.tracks.map(t => tr(TRACKS[t] ?? t)).join("; "), p.tasks.map(t => tr(TASKS[t])).join("; "), p.modalities.map(v => tr(v)).join("; "), collections.find(c => c.paperId === p.id)?.note ?? ""])];
    ...
    const filterPanel = <div className="filter-panel"><div className="search-box"><Search size={18}/><Input aria-label={tr("搜索论文")} placeholder={tr("搜索论文、方法、作者、疾病或 DOI\u2026")} value={search} onChange={e => setSearch(e.target.value)}/></div><div className="filter-row"><SlidersHorizontal size={16}/>{view !== "feed" && <Picker label={tr("感兴趣方向筛选")} value={track} onChange={chooseTrack} options={[["all", "全部感兴趣方向"], ...Object.entries(TRACKS)]}/>}<Picker label={tr("研究任务")} value={task} onChange={setTask} options={[["all", "全部研究任务"], ...Object.entries(TASKS)]}/><Picker label={tr("影像模态")} value={modality} onChange={setModality} options={[["all", "全部模态"], ...modalities.map(x => [x, x] as [
                string,
                string
