...
export function ResearchDashboard({ authenticated, data, teams, collections, filters, onFilters, onDrill, onNavigate, onPaper }: Props) {
    ...
    <button className="dash-analysis-entry" onClick={() => onNavigate("analysis")}><BookOpen size={21}/><span><strong>{tr("查看初步分析")}</strong><small>{tr("研究主题、方法组合与可追溯的论文阅读线索")}</small></span><ArrowUpRight size={18}/></button>
    <div className="dash-controls"><div className="dash-control"><label>{tr("统计时间")}</label><Choice label={tr("仪表盘时间范围")} value={filters.days} onChange={v => change("days", v)} options={[["7", "近 7 天"], ["30", "近 30 天"], ["90", "近 90 天"], ["all", "全部日期"]]}/></div><div className="dash-control"><label>{tr("日期口径")}</label><Choice label={tr("仪表盘日期口径")} value={filters.clock} onChange={v => change("clock", v)} options={[["publication", "来源发布日期"], ["discovery", "平台发现时间"]]}/></div><div className="dash-control"><label>{tr("感兴趣方向")}</label><Choice label={tr("仪表盘感兴趣方向")} value={filters.track} onChange={v => change("track", v)} options={[["all", "全部感兴趣方向"], ...Object.entries(TRACKS)]}/></div><div className="dash-control"><label>{tr("来源")}</label><Choice label={tr("仪表盘数据来源")} value={filters.source} onChange={v => change("source", v)} options={[["all", "全部来源"], ["Europe PMC", "Europe PMC"], ["arXiv", "arXiv"], ["medRxiv", "medRxiv"]]}/></div><Button variant="outline" onClick={exportRows} disabled={!m.rows.length}><ArrowDownToLine size={16}/>{tr("导出分析记录")}</Button></div>
    ...
    <div className="dash-metrics">
      ...
      <Metric label={tr("正式发表")} value={count(m.published.length)} hint={tr("当前范围内的正式发表记录")} icon={Layers3} onClick={() => drill("正式发表", m.published)}/>
      <Metric label={tr("其中预印本")} value={count(m.preprints.length)} hint={tr("当前范围内的预印本记录")} icon={Workflow} onClick={() => drill("预印本", m.preprints)}/>
    ...
    <section className="dash-panel dash-interests"><div className="dash-panel-heading"><div><h2>{tr("感兴趣方向分布")}</h2><p>{tr("使用当前分析范围；同一论文可匹配多个方向，各项不能相加为总量。")}</p></div><span className="dash-unit">{tr("点击查看论文")}</span></div><div className="dash-interest-list">{m.directions.map((direction, index) => <button key={direction.id} disabled={!direction.papers.length} onClick={() => drill(direction.label, direction.papers)}><span className="interest-order">{String(index + 1).padStart(2, "0")}</span><span className="interest-bar-label"><span>{tr(direction.label)}</span><i><i style={{ width: `${direction.papers.length / Math.max(1, ...m.directions.map(d => d.papers.length)) * 100}%` }}/></i></span><b>{count(direction.papers.length)}</b><ArrowUpRight size={14}/></button>)}</div><p className="dash-note">{tr("新增方向从扩展采集后逐步积累；历史记录已重新分类，当前数量不代表完整历史或研究热度。")}</p></section>
    <div className="dash-main-grid">
      <section className="dash-panel dash-trajectory"><div className="dash-panel-heading"><div><h2>{filters.clock === "discovery" ? tr("平台发现记录") : tr("来源日期分布")}</h2><p>{m.monthly ? tr("按月") : tr("按日")}{tr("计数 · 总量含未确认状态记录")}</p></div><span className="dash-unit">{tr("单位：条")}</span></div>
        {m.rows.length ? <ChartContainer className="dash-line-chart" config={{ all: { label: tr("当前范围"), color: "#91a2b3" }, published: { label: tr("正式发表"), color: "#119780" }, preprint: { label: tr("预印本"), color: "#4f84c5" } }}><LineChart accessibilityLayer data={m.trend} margin={{ left: 0, right: 15, top: 14, bottom: 4 }} onClick={(state: any) => {
                if (state?.activeTooltipIndex == null)
                    return;
                const r = m.trend[Number(state.activeTooltipIndex)];
                if (r?.ids.length)
                    onDrill({ label: `${r.date} · ${dateLabel}`, ids: r.ids });
            }}><CartesianGrid vertical={false} strokeDasharray="3 4"/><XAxis dataKey="date" tickFormatter={d => m.monthly ? d : d.slice(5)} minTickGap={28} tickLine={false} axisLine={false}/><YAxis allowDecimals={false} width={35} tickLine={false} axisLine={false}/><ChartTooltip content={<ChartTooltipContent />}/><Line type="linear" dataKey="all" stroke="var(--color-all)" strokeDasharray="4 3" strokeWidth={1.5} dot={m.trend.length === 1} isAnimationActive={false}/><Line type="linear" dataKey="published" stroke="var(--color-published)" strokeWidth={2.5} dot={m.trend.length <= 7} activeDot={{ r: 5 }} isAnimationActive={false}/><Line type="linear" dataKey="preprint" stroke="var(--color-preprint)" strokeWidth={2.5} dot={m.trend.length <= 7} activeDot={{ r: 5 }} isAnimationActive={false}/></LineChart></ChartContainer> : <div className="dash-chart-empty"><Radio size={28}/><strong>{filters.clock === "discovery" ? tr("尚无符合范围的后续发现") : tr("当前范围暂无记录")}</strong><span>{tr("调整时间或来源，或同步最新论文。")}</span></div>}
        <div className="dash-chart-legend"><span><i style={{ background: "#91a2b3" }}/>{tr("当前范围")}</span><span><i style={{ background: "#119780" }}/>{tr("正式发表")}</span><span><i style={{ background: "#4f84c5" }}/>{tr("预印本")}</span></div><details className="dash-series-detail"><summary>{tr("查看日期明细并定位论文")}</summary><div><Table><TableHeader><TableRow><TableHead>{tr("日期")}</TableHead><TableHead>{tr("全部")}</TableHead><TableHead>{tr("正式发表")}</TableHead><TableHead>{tr("预印本")}</TableHead></TableRow></TableHeader><TableBody>{m.trend.map(r => <TableRow key={r.date}><TableCell><button disabled={!r.ids.length} onClick={() => onDrill({ label: `${r.date} · ${dateLabel}`, ids: r.ids })}>{r.date}</button></TableCell><TableCell>{r.all}</TableCell><TableCell>{r.published}</TableCell><TableCell>{r.preprint}</TableCell></TableRow>)}</TableBody></Table></div></details>
